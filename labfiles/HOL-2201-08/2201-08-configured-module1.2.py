...
##### MAIN SCRIPT #####
""" The vRSLCM part is done in the .1 script
##########################################
# CONFIGURE vRSLCM
##########################################
...
if not checkEnterpriseGroups(groupName):
    logMessage = 'The ' + groupName + ' group does not exist in vRA'
    log('\n')
    log(logMessage)
    log('This script cannot proceed. You must complete the steps shown in module 2 of the lab manual.')
    log('- The web-dev-team@corp.local group must be added to vRA before you can proceed.')
    log('Once you have completed those steps, you can run this script again.\n\n')
    quit()
else:
    log('Found the {0} group in vRA. Continuing.'.format(groupName))
